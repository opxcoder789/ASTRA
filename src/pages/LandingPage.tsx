...
// Memoized Featured Carousel — isolated from hero slider re-renders
const FeaturedCarousel = memo(function FeaturedCarousel({ images, btnText }: { images: string[], btnText?: string }) {
...
// Memoized Promo Section — never re-renders on slide change
...
// Memoized Marquee Section — static content, never re-renders
const MarqueeSection = memo(function MarqueeSection() {
  return (
    <section className="fold-effect-section relative min-h-screen bg-black flex items-center justify-center overflow-hidden py-20" style={{ contain: 'content' }}>
...
export default function LandingPage() {
  ...
  // Use refs for video state to avoid re-rendering entire page on mute/play toggle
  const isMutedRef = useRef(true);
  const isPlayingRef = useRef(true);
  const [, forceVideoUI] = useState(0); // minimal re-render trigger for video controls only

  // Initialize with default slide IMMEDIATELY for instant LCP — no waiting for DB
  ...
  useEffect(() => {
    ...
    // Fetch DB slides in background — replace defaults only if data exists
    ...
    // Defer non-critical content fetch to idle time
    ...
    if ('requestIdleCallback' in window) {
      (window as any).requestIdleCallback(() => fetchContent());
    } else {
    ...
    const ctx = gsap.context(() => {
      ...
      ScrollTrigger.create({
        ...
        onUpdate: (self) => {
          const shouldScroll = self.direction === 1 && self.scroll() > 100;
  ...
  // Slide navigation wrapped in startTransition for non-blocking updates
  ...
  // Handle active video playback — direct DOM manipulation, no state updates
  useEffect(() => {
    videoRefs.current.forEach((vid, index) => {
      if (!vid) return;
      if (index === currentSlide) {
        vid.currentTime = 0;
        if (heroSlides[index]?.bg_type === 'video' && isPlayingRef.current) {
          vid.play().catch(() => { });
        } else {
          vid.pause();
        }
  ...
  // Direct DOM video toggle — bypasses React reconciliation entirely
  ...
  return (
    <main className="landing-page bg-[#050505] text-white min-h-screen font-sans no-scrollbar" ref={scrollRef}>

      <Navbar isScrolled={isScrolled} />

      {/* Hero Section — contain: layout style limits repaint to this box */}
      <section className="hero-section relative h-screen w-full overflow-hidden bg-black" style={{ contain: 'layout style' }}>
        {heroSlides.map((slide, index) => (
          <div
            key={slide.id || index}
            className={`absolute inset-0 ${index === currentSlide ? 'opacity-100 z-10' : 'opacity-0 z-0'}`}
            style={{ transition: 'opacity 1s ease-in-out', willChange: 'opacity' }}
          >
            {/* Background Media */}
        ...
        {/* Video Controls — isolated re-render scope */}
        {activeSlide?.bg_type === 'video' && (
          <div className="absolute bottom-8 right-8 z-30 flex gap-3">
            <button
              onClick={togglePlay}
              aria-label={isPlayingRef.current ? 'Pause Background Video' : 'Play Background Video'}
              className="bg-white/10 backdrop-blur-md p-3 rounded-full hover:bg-white/20 transition-colors"
            >
              {isPlayingRef.current ? <Pause size={20} /> : <Play size={20} />}
            </button>
            <button
              onClick={toggleMute}
              aria-label={isMutedRef.current ? 'Unmute Background Video' : 'Mute Background Video'}
              className="bg-white/10 backdrop-blur-md p-3 rounded-full hover:bg-white/20 transition-colors"
            >
              {isMutedRef.current ? <VolumeX size={20} /> : <Volume2 size={20} />}
            </button>
          </div>
        )}
      ...
      {/* Promo Section */}
      <PromoSection content={content} navigate={navigate} />

      {/* Marquee Section */}
      <MarqueeSection />

      {/* Footer Visuals with shader background */}
      <section className="relative bg-black py-32 flex flex-col items-center justify-center overflow-hidden" style={{ contain: 'content' }}>
